...
const setExistingAdminRoleId = guild => {
    logger.info("ROLES", `Setting admin role ID in runtime config for guild "${guild.name}-${guild.id}"`);
    ...
    const adminRoleHighestPosition = guildRoles.cache.reduce((maxPositionRole, role) => role.name === process.env.ADMIN_ROLE_NAME && role.position > maxPositionRole.position ? role : maxPositionRole, { position: -1 });
    ...
    if (adminRoleHighestPosition.position != -1) {
        ...
        config.RUNTIME_CONFIG["ADMIN_ROLE_IDS"][guild.id] = adminRoleHighestPosition.id;
    }
};

const checkAdminRoleExists = guild => {
    logger.info("ROLES", `Checking if admin role exists for guild "${guild.name}-${guild.id}"`);
